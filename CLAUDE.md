...
- **HTTP-tool post-processing** (`internal/httpbridge/postprocess.go`): Registered processors are `chroma_key`, `birefnet_matte`, `binarize_alpha`, `extract_video_frames`, `video_matte`, `check_frame_consistency`, `normalize_frames`, `stitch_spritesheet`. Still-image matting now defaults to **`birefnet_matte` + `binarize_alpha`** (a two-op chain that mirrors the alpha-cleanup half of Walt's video pipeline). `birefnet_matte` shells out to the Python sidecar (`tools/matte/matte.py still`) and runs BiRefNet semantic segmentation on the source PNG, producing an RGBA with a soft probability mask at silhouette edges. `binarize_alpha` (pure-Go, no Python) ports the `floorAlpha` + `binarizeAlpha` helpers out of `normalize_frames` so they're reusable outside Walt's frame-set machinery; with Walt's defaults (`alpha_threshold: "4096"` = 6% floor, `alpha_snap: "32768"` = 50% midpoint) it nukes BiRefNet's soft-edge band that would otherwise show as a faint plate-tinted halo on light surfaces. Cost: ~5–15 s per call for BiRefNet inference, with model load cached after the first call within a process. This replaced the prior in-Go `chroma_key` default after the 2026-05-04 fire-wizard test demonstrated that `chroma_key`'s score-based magenta keyer fails completely under "subject-palette bleed" (Nano Banana painting the matte in maroon / dusty-rose / off-white instead of `#FF00FF`) while BiRefNet handled it cleanly regardless of plate color. The `chroma_key` op (`(min(R,B)−G)/255` keyer + smoothstep edge band + magenta despill, plus opt-in `kill_shadow` HSV second pass for darkened-magenta shadow remnants) remains a registered processor — sub-second, deterministic, no model dependency — and is still preferable for any caller whose sprites are reliably rendered against true `#FF00FF` magenta and who needs the despill cleanup `birefnet_matte` lacks. RebelTower's Miyazaki persona moved off `chroma_key` and onto BiRefNet for both `nano-banana` (fresh) and `nano-banana-edit` (image-to-image) — the prior `nano-banana-rematte` recovery tool was deleted as redundant since the recovery case no longer exists. Residual algorithmic gap vs. `chroma_key` despill: `birefnet_matte` + `binarize_alpha` has no per-pixel plate-contamination cleanup, so kept-but-edge pixels whose RGB carries plate contamination remain tinted (faint colored bleed at silhouette edges visible at full resolution). Walt's video pipeline shares this gap and compensates via the 4× downscale-averaging in `normalize_frames`'s scale step (16-pixel-neighborhood Lanczos averaging dilutes per-pixel contamination); stills don't get that dilution but the bleed is empirically minor and not visible at sprite-sheet display scale. Adding a despill pass to the still chain (e.g. a `despill` op that operates on RGB-near-plate-color regardless of color, configurable plate hue) is the obvious follow-up if the residual bleed becomes unacceptable for any future preset.
- **Image-to-image edit pattern** (Miyazaki's `nano-banana-edit` http_tool, RebelTower's `agents/miyazaki.md`): companion tool to `nano-banana` that targets the same `gemini-2.5-flash-image:generateContent` endpoint with a multimodal request — a `text` part plus an `inline_data` part carrying the existing artifact base64-encoded (mirrors Walt's `veo` `image_file` input pattern; bridge handles file→base64 via `input_types.source_image: { type: file, encoding: base64, max_size_mb: 10 }`). Used for user-initiated reruns (Prism gate selective-revise OR chat fix-requests on an existing artifact) so we don't pay full fresh-generation cost just to strip a defect. Same `birefnet_matte` + `binarize_alpha` post-process chain as fresh generation. Tool selection lives in Miyazaki's persona body under "Edit vs. fresh generation" — heuristic favors edit-mode when ambiguous because it's half the cost and preserves approved pixels. No automatic image-to-image second-pass on every generation; strictly opt-in via user-initiated rerun. Video matting (`video_matte`) and style-drift (`check_frame_consistency`) still shell out to the Python sidecar at `tools/matte/matte.py` (BiRefNet + SAM 2 + DINOv2). `video_matte` degrades to per-frame BiRefNet if SAM 2 fails and surfaces the mode in the result message. `check_frame_consistency` is advisory — it flags outlier frames in `state["drift_frames"]` / `state["drift_report"]` but never fails the pipeline. Sidecar discovery uses `MATTE_PYTHON` + `MATTE_SCRIPT` env vars (see `locateMattePython`); missing sidecar yields `PostProcessSkipped`, not `Failed`. See [`tools/matte/README.md`](tools/matte/README.md).
- **Color-op symmetry rule (still ↔ animation handoff).** "Color-touching" = any post-process op that modifies subject-pixel **RGB**, as opposed to alpha-only or layout-only. Today's alpha-only / layout-only ops: `birefnet_matte`, `binarize_alpha`, `video_matte`, `extract_video_frames`, `stitch_spritesheet` (none of these read or write subject RGB). Today's color-touching ops: `normalize_frames`'s 4× Lanczos downscale step (Walt only, incidental — 16-pixel-neighborhood averaging modifies RGB on every pixel including subject interior, which doubles as a free magenta-cast despill at silhouette edges); deprecated: `chroma_key`'s explicit despill loop. **Rule:** any new RGB-modifying op (despill, recolor, tone-map, color-grade, etc.) must be added to **both** Miyazaki's still chain (`nano-banana` + `nano-banana-edit` `post_process`) **and** Walt's animation chain (`veo` / `veo-animate` `post_process`) — otherwise the still→animation handoff will introduce a visible color shift between sprite and idle loop, since a sprite produced by one chain is the literal first-frame input to the other. The 2026-05-04 color-drift analysis showed today's ~4 RGB-unit median L2 between stills and Veo regenerations is dominated by Veo regeneration noise, not by the existing `normalize_frames` asymmetry, so no symmetric despill is required today; this rule exists to prevent a future contributor from breaking that.
