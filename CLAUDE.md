...
- **HTTP-tool post-processing** (`internal/httpbridge/postprocess.go`): Registered processors are `remove_background`, `extract_video_frames`, `video_matte`, `check_frame_consistency`, `normalize_frames`, `stitch_spritesheet`. Matting (`remove_background`, `video_matte`) and style-drift (`check_frame_consistency`) shell out to the Python sidecar at `tools/matte/matte.py` (BiRefNet + SAM 2 + DINOv2). The legacy `rembg` backend has been deleted; `remove_background` is single-backend now (`remove_background: config: {}` is valid). `video_matte` degrades to per-frame BiRefNet if SAM 2 fails and surfaces the mode in the result message. `check_frame_consistency` is advisory — it flags outlier frames in `state["drift_frames"]` / `state["drift_report"]` but never fails the pipeline. Sidecar discovery uses `MATTE_PYTHON` + `MATTE_SCRIPT` env vars (see `locateMattePython`); missing sidecar yields `PostProcessSkipped`, not `Failed`. See [`tools/matte/README.md`](tools/matte/README.md).
