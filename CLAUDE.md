...
- **Specialist agent profiles (Phase 4)**: Named profiles (architect, coder, tester, debugger, explorer) with different prompt templates, tool configs, and model settings. The orchestrator selects a profile when dispatching via `profile` field on the `dispatch` action. Reviewer-failed retries automatically use the `debugger` profile. The `explorer` profile is used internally by plan mode for parallel research subagents (read-only, denied Write/Edit/MultiEdit).
