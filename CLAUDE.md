...
- **Orchestrator modes (Phase 3a+)**: The orchestrator supports three channel modes: **Plan** (markdown-only planning using `buildPlanSystemPrompt` which omits JSON actions and HTML display instructions — plans saved to `~/.anthem/plans/{project-slug}/`, a `[plan-card]` sent to chat after finalization; plan mode NEVER creates GitHub issues), **Build** (reads approved plan, MUST emit `create_subtasks` to create GitHub issues with `todo` label for executor dispatch — this is the deliberate moment issues enter the tracker), and **Agent** (default — full JSON actions with plan context injected via `ActivePlan`/`PlanHistory` in StateSnapshot, can also edit code directly via Claude Code). The Go daemon validates and executes all actions. Falls back to Phase 2 mechanical dispatch on failure.
...
- Plan mode restricted to markdown: plan mode uses `buildPlanSystemPrompt` (not `buildSystemPrompt`) which omits the JSON action schema, HTML display instructions, and `create_subtasks` — preventing the LLM from defaulting to HTML or emitting actions. `handlePlanOutput` routes to two paths: `anthem-plan` fenced block → `finalizePlan` (saves markdown, broadcasts display, sends PlanCard), anything else → `sendFollowUp` (conversational text reply). `planModePromptSuffix` and `synthesisPromptSuffix` explicitly forbid JSON actions, HTML, and display artifacts. All plans are saved to `~/.anthem/plans/` ensuring plan context is available for agent/build mode handoff. Dead code removed: `finalizeHTMLPlan`, `isSubstantialOutput`, `buildPlanCardFromHTML`, HTML extraction helpers, `sanitizePlanOutput`, `planStreamFilter`.
