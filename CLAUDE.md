...
- **Orchestrator modes (Phase 3a+)**: The orchestrator supports three channel modes: **Plan** (format-agnostic planning — produces HTML artifacts for rich visual analysis or markdown artifacts saved to `~/.anthem/plans/{project-slug}/`, no JSON actions — plan mode NEVER creates GitHub issues; a `[plan-card]` is sent to chat for both formats), **Build** (reads approved plan, MUST emit `create_subtasks` to create GitHub issues with `todo` label for executor dispatch — this is the deliberate moment issues enter the tracker), and **Agent** (default — full JSON actions with plan context injected via `ActivePlan`/`PlanHistory` in StateSnapshot, can also edit code directly via Claude Code). The Go daemon validates and executes all actions. Falls back to Phase 2 mechanical dispatch on failure.
...
- **Specialist agent profiles (Phase 4)**: Named profiles (architect, coder, tester, debugger, explorer, security-explorer, test-explorer) with different prompt templates, tool configs, and model settings. The orchestrator selects a profile when dispatching via `profile` field on the `dispatch` action. Reviewer-failed retries automatically use the `debugger` profile. The `explorer` profile is used internally by plan mode for parallel research subagents (read-only, denied Write/Edit/MultiEdit). Profiles can reference MCP servers and skills from the global registries via `mcp_refs` and `skill_refs` fields.
...
- **Orchestrator codebase awareness (Phase 4)**: The orchestrator agent uses Claude Code's built-in tools (Read, Grep, Glob) during planning instead of working blind from static docs. The prompt role changes from "stateless allocator" to "intelligent orchestrator with codebase access." `orchestrator.max_turns` config (default 10) controls exploration depth for agent/build mode. `orchestrator.plan_max_turns` (default 25) gives plan mode a fallback turn budget for simple requests. Plan mode is read-only (Write/Edit/MultiEdit denied).
- **Explorer subagent architecture (Post-Phase 4)**: Plan mode uses a three-phase pipeline: (1) **Scout** — the plan agent reads the file tree, identifies 1-5 areas needing deep research, emits explore requests (8 turns). (2) **Explore** — the Go daemon spawns parallel Claude Code processes (one per area, `explorer_max_turns` default 10, capped at `max_explorers` default 5), each with a focused research question and read-only tools. Results collected via goroutines + WaitGroup. (3) **Synthesize** — the plan agent receives all explorer findings and produces an evidence-based plan. If scout returns 0 explores (trivial request), falls back to single-run ConsultPlan. Explorer costs tracked under `__explorer__` synthetic task ID.
...
- Plan mode deep research: separate `plan_max_turns` config (default 25) for fallback single-run mode. Write tools denied in plan mode via `DeniedTools` on `RunOpts`.
- Explorer subagent architecture: three-phase plan pipeline (scout → parallel explore → synthesize). `explorer_max_turns` (default 10) and `max_explorers` (default 5) config. `explorer` profile added to default agent profiles. `runExplorers` method uses goroutines + WaitGroup for parallel Claude Code processes. Cost tracked under `__explorer__` synthetic task ID. Fallback to single-run ConsultPlan when scout returns 0 explores.
- Profile-based agent harness architecture: three-layer "Registry + Reference" design. Layer 1 — MCP servers defined as a named map on `AgentConfig` (capabilities registry). Layer 2 — Skills as a global list on `AgentConfig` (knowledge registry). Layer 3 — Profiles compose by reference via `mcp_refs` (MCP server names) and `skill_refs` (skill identifiers). `internal/harness/` package writes `.mcp.json` and copies skills to `.claude/skills/` in each workspace before agent launch (Claude Code auto-discovers both). Unified `resolveProfile()` method on Orchestrator merges base config + profile overrides + MCP/skills, used by both task dispatch and explorer subagents. `focusToProfile()` maps explorer focus categories to specialized profiles (`security` → `security-explorer`, `tests` → `test-explorer`). Adding a new agent type requires only a YAML profile entry — no code changes.
- Embedded core skills: 6 SKILL.md packages compiled into the binary via `go:embed` at `internal/harness/skills/`. Skills are extracted to agent workspaces at `.claude/skills/` before launch. Core skills: `test-verifier` (Go test pyramid, coverage), `gh-issue-verifier` (evidence-based issue verification), `code-review` (4-category structured review), `tdd-classicist` (language-agnostic TDD methodology), `go-cli` (CLI doctrine), `commit-hygiene` (conventional commits). Global baseline skills (`code-review`, `gh-issue-verifier`) apply to all agents. Profile-specific skills wired via `skill_refs` — e.g. `tester` gets `test-verifier` + `tdd-classicist`, `coder` gets `commit-hygiene` + `go-cli`. Embedded skills tried first, filesystem fallback to `~/.anthem/skills/` for user-added skills.
- Explorer prompt sharpening: `test-explorer` profile prompt updated to go beyond zero-coverage files — explicitly instructs agent to look inside partially tested files, distinguish route/endpoint tests from service/helper tests, and flag security-critical code lacking tests. `BuildExplorerPrompt` in `orchagent.go` now includes focus-specific instruction blocks: `tests` focus adds route-vs-service and security-boundary checks, `security` focus adds dedicated audit instructions for input validation, path traversal, auth middleware with test verification. Scout prompt (`scoutPromptSuffix`) includes a cross-focus rule: test coverage requests always spawn a security explorer alongside test explorers.
- Plan-card in chat: after plan finalization, the orchestrator sends a `[plan-card]{...}[/plan-card]` JSON message to the channel. Prism renders this as a structured UI card with title, task list, overview, "View Plan" link (opens artifact pane), model dropdown, and "Build" button (sends `[system:build]` message). `buildPlanCard` constructs the JSON from plan metadata (title, overview, tasks extracted from markdown). `finalizePlan` in `orchestrator.go` broadcasts the plan as a `kind:plan` display artifact and then sends the plan-card as a follow-up text message.
- Format-agnostic plan output: plan mode can return three types of output — conversational text, rich HTML (for data-rich analysis with metrics/charts/tables), or structured markdown (via `anthem-plan` fenced blocks). `handlePlanOutput` in `orchestrator.go` routes based on content: (1) `extractPlanBlock` finds an `anthem-plan` block → `finalizePlan` saves markdown to `~/.anthem/plans/`, broadcasts `kind:plan` display, sends PlanCard. (2) `isSubstantialOutput` detects significant HTML/analysis (>500 chars with HTML markers or plan-structure markers) → `finalizeHTMLPlan` broadcasts `kind:html` display artifact, sends PlanCard with empty `planPath`. (3) Short conversational text → `sendFollowUp` plain reply. HTML extraction helpers (`extractHTMLPlanTitle`, `extractHTMLPlanTasks`, `extractHTMLPlanOverview`, `stripHTMLTags`) parse plan metadata from HTML for PlanCard generation via `buildPlanCardFromHTML`. Plan mode prompts (`planModePromptSuffix`, `synthesisPromptSuffix`) updated to guide the LLM to choose the best format: HTML for rich reports with metrics, markdown for simple task plans, plain text for conversation. Prism's `PlanCard.tsx` handles both `planPath: ""` (HTML artifact) and populated `planPath` (markdown file) — "View Plan" opens the artifact pane in either case, "Download" creates a markdown blob fallback for HTML artifacts.
- Lean prompt no-tools constraint: added explicit "Do not output raw tool calls" instruction to the lean path system prompt to prevent Fast mode from leaking tool-call syntax in responses.
...
- `display` frames: `{"type":"display","component":{...},"thread":"<id>"}` for A2UI structured visual content. Display components include `kind:plan` (markdown plan artifacts) and `kind:html` (rich HTML analysis artifacts).
...
- `plan-card` messages: `[plan-card]{"title":"...","overview":"...","tasks":[...],"planPath":"..."}[/plan-card]` embedded in `res` frame text. Prism's `PlanCard.tsx` parses these and renders a structured card with title, task list, "View Plan" (opens artifact pane), model dropdown, and "Build" button. `planPath` is empty for HTML artifacts, populated for saved markdown files.
