...
- **Mode router**: `[system:<mode>]` tag in inbound messages selects the mode. Legacy tags (`fast`, `build`, `status`) remap for backward compatibility. Untagged messages fall back to `Orchestrator.CurrentMode` (Chat by default; Loop when a tracker is configured in `WORKFLOW.md`).
